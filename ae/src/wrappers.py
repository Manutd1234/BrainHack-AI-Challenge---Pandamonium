"""
Training-time env wrappers.

Two ingredients:

1.  `SB3AECWrapper` — turns the PettingZoo AEC `Bomberman` into a single-agent
    Gymnasium env that SB3 can drive. The "learner" controls agent_0. All other
    agents are controlled by an `opponent_pool` (frozen snapshots + scripted
    bots + random). This is the core of league self-play.

2.  `RewardShaper` — adds dense potential-based shaping so the learner gets
    signal before it stumbles into a +50 base-destroy. The shaping decays to
    zero over training so the final policy is unbiased w.r.t. the true reward.

Both are deliberately decoupled — you can keep shaping on for phase 1, anneal
in phase 2, turn off entirely in phase 3.
"""
...
from .obs_utils import (
    AGENT_VC_H, AGENT_VC_W, BASE_VC_S, GRID_SIZE, N_CHANNELS, NUM_ACTIONS,
    SCALAR_DIM, pack_for_sb3,
)
...
# --------------------------------------------------------------------------- #
class OpponentPool:
    """Holds frozen snapshots + a few scripted/random opponents.

    `sample()` returns a callable: obs -> action.
    """
    def __init__(self, max_snapshots: int = 8):
        self._snapshots: deque = deque(maxlen=max_snapshots)
        self._bots: List[Callable[[Dict[str, Any]], int]] = [
            self._random_legal_bot,
            self._collector_bot,   # walks toward unvisited cells, sometimes bombs
        ]
    ...
    def sample(self) -> Callable[[Dict[str, Any]], int]:
        # 60% snapshot, 40% scripted/random (keeps the league from collapsing)
    ...
    def _collector_bot(obs: Dict[str, Any]) -> int:
        """A weak heuristic: prefer FORWARD if legal, occasionally bomb."""
        mask = np.asarray(obs.get("action_mask", [1]*NUM_ACTIONS))
        # bomb sometimes if it's legal and we have bombs
        if mask[5] and obs.get("team_bombs", 0) > 0 and random.random() < 0.05:
            return 5
        for a in (0, 2, 3, 1, 4):           # forward, sideways, back, stay
            if mask[a]:
                return a
        return 4
...
def _predict_with_mask(model, obs: Dict[str, Any]) -> int:
    ...
    try:
        ...
        return 4  # STAY on opponent failure — don't crash training
...
# --------------------------------------------------------------------------- #
class RewardShaper:
    """Potential-based shaping. `coef` should be annealed toward 0 across training."""
    ...
    def _potential(obs: Dict[str, Any]) -> float:
        ...
        Rewards:
          + having more HP than baseline
          + having more team resources
          + having more bombs available
        Penalises:
          - being far from your base when low HP (encourages strategic retreat)
        """
        health = float(obs.get("health", 0)) / 60.0          # [0,1]
        resources = float(obs.get("team_resources", 0)) / 10.0
        bombs = float(obs.get("team_bombs", 0)) / 10.0

        loc = np.asarray(obs.get("location", [0, 0]), dtype=np.float32)
        base_loc = np.asarray(obs.get("base_location", [0, 0]), dtype=np.float32)
        dist_to_base = np.linalg.norm(loc - base_loc) / (GRID_SIZE * 1.4)
        retreat_pressure = max(0.0, 0.3 - health) * dist_to_base   # only when hurt
...
class SB3AECWrapper(gym.Env):
    """Single-agent Gymnasium env wrapping the multi-agent PettingZoo Bomberman.

    Learner controls `learner_agent_id`. Other agents are controlled by
    `opponent_pool`. Reward shaping is applied to the learner only.
    """
    ...
    def reset(self, *, seed: Optional[int] = None, options=None):
        ...
        # fresh opponent assignment each episode
        self._opponent_policies = {
            a: self.opponent_pool.sample()
            for a in self._aec.agents if a != self.learner_id
        }
        self.shaper.reset()
    ...
    def step(self, action: int):
        ...
        # accumulate reward for the learner across opponents' turns
        ...
        for agent in self._aec.agent_iter():
            ...
            if term or trunc:
                self._aec.step(None)
                continue
            opp_action = self._opponent_policies.get(agent,
                                                     self.opponent_pool._random_legal_bot)(obs)
            # respect the opponent's own mask
            mask = np.asarray(obs.get("action_mask", [1]*NUM_ACTIONS))
        ...
        # ran out of iter without learner turn -> episode done
