...
Total: 30M steps. If you only have budget for 20M+10M, skip Phase 3.
...
def make_sb3_env(opponent_pool: OpponentPool, shaping_coef: float, novice: bool = True) -> Callable:
    def _thunk():
...
class SnapshotCallback(BaseCallback):
    def __init__(self, opponent_pool: OpponentPool, snapshot_every: int = 500_000, save_dir: str = "models/snapshots"):
        super().__init__()
...
def train_phase(phase: int, total_envs: int, device: str, novice: bool, resume: str | None, out_dir: Path) -> str:
    cfg = PHASES[phase]
    ...
    vec_env = SubprocVecEnv([make_sb3_env(opponent_pool, cfg["shaping"], novice=novice) for _ in range(total_envs)])
...
def main():
    ...
    ap.add_argument("--novice", action="store_true", default=True, help="Novice = fixed map.")
    ...
    if args.phase == "all":
        ...
        print(f"\n  Promoted {final} -> {ae_policy}")
    else:
        path = train_phase(int(args.phase), args.total_envs, args.device, args.novice, args.resume, out_dir)
        if args.phase == "3":
            ae_policy = out_dir / "ae_policy.zip"
