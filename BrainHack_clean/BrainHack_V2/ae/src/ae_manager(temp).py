...
# CHECKPOINT_PATH = "/app/model/v5_absolute_best.zip"
BASE_ATTACK_SCORE = float(os.getenv("AE_BASE_ATTACK_SCORE", "105.0"))
AGENT_ATTACK_SCORE = float(os.getenv("AE_AGENT_ATTACK_SCORE", "30.0"))
...
class AEManager:
    ...
    def __init__(self) -> None:
        ...
        # self.policy_mode = os.getenv("AE_POLICY_MODE", "ppo_first").strip().lower()
    ...
    #     if self.policy is not None:  # no policy_mode check
    #         action = self._policy_act(observation, action_mask)
    #         if self._is_legal(action, action_mask):
    #             return self._finish_action(action, observation)
