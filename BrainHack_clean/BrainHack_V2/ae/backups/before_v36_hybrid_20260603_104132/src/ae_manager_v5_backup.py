...
COLLECTIBLE_VALUE = {"mission": 10.5, "resource": 4.8, "recon": 1.4}
...
class PlannerState:
    ...
    def update(self, obs):
        ...
        # Action mask also tells us whether current forward/back edges are blocked.
        mask_now = _mask(obs)
    ...
    def _best_path(self):
        ...
        for target in candidates:
            ...
            if kind:
                score += COLLECTIBLE_VALUE.get(kind, 1.0) * 4.8
            if target not in self.seen_cells:
                score += 9.0
            if target in self.predicted_bases:
                score += 18.0 + min(20.0, max(0, self.obs_step - 70) / 4.0)
